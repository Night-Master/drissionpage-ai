...
class ChromiumBase(BasePage):
    ...
    def agent(self):
        if self._agent is None:
            ...
            self._agent = DrissionPageAgent(self)
        return self._agent

    def aiAct(self, prompt, options=None):
        return self.agent.aiAct(prompt, options=options)

    def ai(self, prompt, options=None):
        return self.agent.ai(prompt, options=options)

    def aiTap(self, locate, options=None):
        return self.agent.aiTap(locate, options=options)

    def aiHover(self, locate, options=None):
        return self.agent.aiHover(locate, options=options)

    def aiInput(self, locate, opt=None, options=None):
        return self.agent.aiInput(locate, opt=opt, options=options)

    def aiKeyboardPress(self, locate=None, opt=None):
        return self.agent.aiKeyboardPress(locate=locate, opt=opt)

    def aiScroll(self, locate=None, opt=None):
        return self.agent.aiScroll(locate=locate, opt=opt)

    def aiDoubleClick(self, locate, options=None):
        return self.agent.aiDoubleClick(locate, options=options)

    def aiRightClick(self, locate, options=None):
        return self.agent.aiRightClick(locate, options=options)

    def aiAsk(self, prompt, options=None):
        return self.agent.aiAsk(prompt, options=options)

    def aiQuery(self, data_demand, options=None):
        return self.agent.aiQuery(data_demand, options=options)

    def aiQueryImages(self, prompt, options=None):
        return self.agent.aiQueryImages(prompt, options=options)

    def aiBoolean(self, prompt, options=None):
        return self.agent.aiBoolean(prompt, options=options)

    def aiNumber(self, prompt, options=None):
        return self.agent.aiNumber(prompt, options=options)

    def aiString(self, prompt, options=None):
        return self.agent.aiString(prompt, options=options)

    def aiAssert(self, assertion, error_msg=None, options=None):
        return self.agent.aiAssert(assertion, error_msg=error_msg, options=options)

    def aiLocate(self, locate, options=None):
        return self.agent.aiLocate(locate, options=options)

    def aiWaitFor(self, assertion, options=None):
        return self.agent.aiWaitFor(assertion, options=options)

    def runYaml(self, yaml_script_content):
        return self.agent.runYaml(yaml_script_content)

    def setAIActContext(self, ai_act_context):
        return self.agent.setAIActContext(ai_act_context)

    def evaluateJavaScript(self, script):
        return self.agent.evaluateJavaScript(script)

    def recordToReport(self, title=None, options=None):
        return self.agent.recordToReport(title=title, options=options)

    def freezePageContext(self):
        return self.agent.freezePageContext()

    def unfreezePageContext(self):
        return self.agent.unfreezePageContext()
