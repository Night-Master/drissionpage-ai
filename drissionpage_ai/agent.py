...
from .context import build_context, cap_screenshot_for_model, normalize_screenshot_to_css_pixels
from .extractor import AIExtractor
from .locator import AILocator, _bbox_center, _interpret_model_bbox, _scale_bbox
from .model import OpenAICompatibleModel, _resolve_debug_dir
...
class DrissionPageAgent(object):
    ...
    def aiInput(self, locate, value, clear=True, options=None):
        if isinstance(value, dict):
            raise ValueError("aiInput() no longer takes an options dict as value; "
                             "use aiInput(locate, value, clear=True, options={...}).")
        if value is None:
...
def _interpret_at_bbox(bbox, prefer_normalized, normalized, model_name=''):
    """Interpret a tap/drag bbox against the screenshot the model actually saw.

    Explicit 'css' coordinates are read directly against the CSS-sized screenshot.
    Auto-detected/normalized coordinates are read against the 1080p-capped model
    image and scaled back to CSS viewport coordinates via model_scale (a no-op
    when the screenshot was not capped).
    """
    if prefer_normalized is False:
        return _interpret_model_bbox({'bbox': bbox}, normalized['size'], model_name=model_name,
                                     prefer_normalized=False)
