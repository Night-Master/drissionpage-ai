...
def _debug_dir(debug):
    if isinstance(debug, dict) and debug.get('dir'):
        return debug['dir']
    return _resolve_debug_dir(None)


def _resolve_debug_dir(configured):
    if configured:
        return configured
    return getenv('DP_AI_DEBUG_DIR') or _default_debug_dir()


def _default_debug_dir():
    """'debug_log' folder next to the first caller script outside the DrissionPage
    package; falls back to the system temp dir when no real script file is found."""
    ...
    try:
        while frame:
            try:
                ...
                frame = frame.f_back
                continue
            if resolved.is_file() and package_root not in resolved.parents:
                return str(resolved.parent / 'debug_log')
            frame = frame.f_back
    finally:
        del frame
    return str(Path(gettempdir()) / 'drissionpage_ai_debug')
